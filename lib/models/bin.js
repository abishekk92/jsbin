...
module.exports = Observable.extend({
  ...
  created: function (params, fn) {
    var timer = metrics.createTimer(metric_prefix + 'created');
    var original = fn;
    fn = function () {
      timer.stop();
      original.apply(this, [].slice.call(arguments));
    };
    this.store.getCreatedTime.call(this.store, params, fn);
  },
  ...
  createUrl: function(fn){
    var timer = metrics.createTimer(metric_prefix+'create_url');
    this.store.generateBinId(function(err,id){
      if(err){
        return fn(err);
      }
  ...
  set_status: function(status,bin,fn){
    var timer = metrics.createTimer(metric_prefix+'set_status');
    var original = fn;
    fn = function(){
      timer.stop();
      original.apply(this,[].slice.call(arguments));
    };
    this.store.set_status.call(this.store,status,bin,fn);
  },
  get_status: function(bin,fn){
    var timer= metrics.createTimer(metric_prefix+'get_status');
    var original = fn;
    fn = function(){
      timer.stop();
      original.apply(this,[].slice.call(arguments));
    };
    this.store.get_status.call(this.store,bin,fn);
  }
});
