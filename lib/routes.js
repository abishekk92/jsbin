...
module.exports = function (app) {
  ...
  }, binHandler, sessionHandler, apiHandler,errorHandler;
  ...
  apiHandler = new handlers.APIHandler(sandbox);

  // Special redirects (to external sources)
  app.get(['/video', '/videos', '/tutorials'], function (req, res) {
    res.redirect('http://www.youtube.com/playlist?listgetDe=PLXmT1r4krsTooRDWOrIu23P3SEZ3luIUq');
  });
  ...
  //Before filter for API to make sure all the requests are authorized
  app.all('/api/*',passport.authenticate('bearer',{session:false}));

  // API Endpoints
  app.get('/api/create',apiHandler.createUrl);
  app.get('/api/open/:url',apiHandler.openUrl);
  app.get('/api/close/:url',apiHandler.closeUrl);
  ...
  app.get('/login',function(req,res){
    res.render("login",{
      "jsbin": "{\"root\":\"http://localhost:3000\",\"token\":"+"\""+req.session._csrf+"\""+"}"
  });
});
  ...
  //Before fitlers
  // app.all('/*',sessionHandler.logInFromSession,sessionHandler.loadUser);
  // List
  ...
  app.all('/:bin/*/(edit|watch)',apiHandler.is_edit_allowed);
  ...
  // app.get('/:name/last/edit', binHandler.getLatestForUser, binHandler.getBin); Deprecate the feature
  app.get('/:name/last/watch', binHandler.getLatestForUser, binHandler.live,binHandler.getBinPreview);
