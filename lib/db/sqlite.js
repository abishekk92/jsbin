...
module.exports = utils.inherit(Object, {
  ...
  getCreatedTime: function(params,fn){
    var values = [params.id, params.revision],
        _this = this;

    this.connection.get(templates.getCreatedTime, values, function (err, result) {
      if (err) {
        return fn(err);
      }
  ...
  set_status: function(status,url,fn){
      this.connection.run(templates.set_status,[status,url],function(err){
        if(err){
          return fn(err);
        }
        else{
  ...
  get_status: function(url,fn){
    this.connection.get(templates.get_status,[url],function(err,result){
      if(err){
        return fn(err);
      } 
      else if(result){
        result=result.is_alive;
      }
        fn(null,result);
    });
  }
